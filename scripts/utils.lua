...
function Utils.findSiloByUnitNumber(unit_number)
  ...
  for index = 1,  #storage.world.city_names do
    local check_city = storage.world.cities[storage.world.city_names[index]]
...
function Utils.calculateMaxLaunches( pre_place_silo )
  ...
  if pre_place_silo == Config.ALL and storage.settings.startup.em_pre_place_limit.value then
    return 1
  end
...
function Utils.calculateTotalLaunches()
  ...
  local pre_place_silo = storage.settings.startup.em_pre_place_silo.value
  ...
  elseif pre_place_silo == Config.NONE or pre_place_silo == Config.SINGLE then
    total_launches = storage.world.rocket_silo.launches_this_silo
  end
...
function Utils.calculateRemainingLaunches()
  ...
  if not storage.settings then return 0 end
  local pre_place_silo = storage.settings.startup.em_pre_place_silo.value
  ...
  if pre_place_silo == Config.ALL then
    ...
    remaining_launches =  storage.world.rocket_silo.required_launches - completed_launches_count
  elseif pre_place_silo == Config.SINGLE then
    remaining_launches = storage.world.rocket_silo.required_launches - storage.world.rocket_silo.launches_this_silo
  end
...
-- For ver 1.6.0, silo data is moved from storage to 'silo_city'.
function Utils.moveSiloData()
  if storage.world.rocket_silo == nil then
  ...
  if storage.world.silo_city.rocket_silo == nil then
    storage.world.silo_city.rocket_silo = storage.rocket_silo
  end
end
...
function Utils.saveStartupSettings()
  storage.settings = { startup = {} }
  storage.settings.startup.em_pre_place_silo = settings.startup.em_pre_place_silo
  storage.settings.startup.em_pre_place_limit = settings.startup.em_pre_place_limit
  storage.settings.startup.em_source_teleporters_require_power = settings.startup.em_source_teleporters_require_power
  storage.settings.startup.em_dest_teleporters_require_power = settings.startup.em_dest_teleporters_require_power

  if settings.startup["em_team_coop"].value == true then
    storage.settings.startup.em_pre_place_silo.value = Config.ALL
  end
end
...
function Utils.print(msg, skip_game_print)
  if settings.startup.em_dev_mode and settings.startup.em_dev_mode.value then
