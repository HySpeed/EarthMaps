...
local function setupForDevMode( player )
  local worldgen = storage.worldgen
  local world = storage.world
  ...
  armor.put { name = "fission-reactor-equipment" }
  armor.put { name = "fission-reactor-equipment" }
  ...
  if Utils.getStartupSetting( "em_team_coop" ) then
    for _, city in pairs( storage.world.cities ) do
      ...
      for _, force in pairs( game.forces ) do
        player.force = force
        force.add_chart_tag( storage.world.surface, tag )
      end
    end
  end

  player.force = game.forces[storage.world.spawn_city.name]
  if settings.startup.em_dev_mode then setupForDevMode( player ) end
